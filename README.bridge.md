...
Cursor の能力を最大限引き出すため、プロンプト作成時は以下を意識してください：

- ✅ **What（何を）を明確に**、**How（どのように）は Cursor に任せる**
- ✅ **細かいコード指示は避ける**（Cursor が最適な実装を選択）
- ✅ **コマンド列挙は最小限**（依存関係・ビルド手順は自動推論）
- ✅ **目的と制約を伝え、実装の詳細は Cursor の判断を尊重**

詳細は `prompts/_SCHEMA.md` の「プロンプト作成時の原則」を参照してください。

### 🔄 運用フロー（効率化版）

1. **HQ**: プロンプトを `prompts/` に配置
2. **Cursor**: 実装 → コミットまで（**プッシュ・レポートは待機**）
3. **ユーザー**: 実装内容を確認
4. **ユーザー**: 明示的に「プッシュして」「レポート書いて」と指示
5. **Cursor**: 指示に従ってプッシュ・レポート生成
6. **HQ**: レポートを確認して次のステップを討論
